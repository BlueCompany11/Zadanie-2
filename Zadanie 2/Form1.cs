...
namespace Zadanie_2
{
    public partial class Form1 : Form
    {
        ...
        private async Task<List<ExcelObject>> CreateobjectsFromExcel(List<string> headers)
        {
        ...
        private async Task<int> DoTask()
        {
            List<string> headers = new List<string>() { "Nazwa", "ID", "Cena", "Pozycja", "Poziom", "Opis", "Nr Zamówienia" };
            var x = await CreateobjectsFromExcel(headers);
            return 5;
        }

        private async void buttonGetFile_Click(object sender, EventArgs e)
        {
            var result = await Task.Run(() => DoTask());
            //DialogResult result = openFileDialog.ShowDialog();
            //if (result == DialogResult.OK)
            //{
            //    string file = openFileDialog.FileName;
